...
- Queue write operations in strict-rate deployments. Submit/cancel should run through one frontend queue with a conservative spacing, persisted last-call timestamp, and rate-limit retry; never let users repeatedly click into `access_key` throttling.
