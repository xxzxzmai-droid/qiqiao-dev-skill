...
- For half-hour reservation UIs, a single free slot should be immediately bookable. Treat the first slot click as a ready half-hour range, and let a later free slot click extend the end time; do not force users to click two different slots for a 30-minute booking.
- For mobile reservation UX, avoid putting room selection, schedule, form, confirmation, and history on one page. Use a staged flow: room list -> schedule/time selection -> required info form -> confirmation -> my reservations. Keep the PC page consistent with the same state machine, even if it has more horizontal space.
- For reservation edits, expose a backend `updateReservation` method only for safe business fields such as purpose, reserver, and free-text attendees. Treat room/date/time changes as cancel-and-create unless an explicit conflict-checked update path has been implemented.
- Validate required reservation fields such as purpose both before the confirmation step and again in `server-code.js`; frontend-only validation is not enough for Qiqiao custom pages.
