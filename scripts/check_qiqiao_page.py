...
def main():
    ...
    if js:
        ...
        preview_guard = "preview" in js.lower() and ("execute" in js or "REST.API" in js)
    ...
    secret_like = re.search(r"(secret|password|passwd|authorization\s*:|bearer\s+[a-z0-9._-]{12,}|x-auth0-token\s*[:=]\s*['\"][^'\"]{8,})", combined, re.I)
    add(results, "warn" if secret_like else "pass", "secrets.scan", "No obvious hardcoded secret patterns")
